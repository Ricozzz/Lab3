...
void drawline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	      ...
	      {    setpixel(buff,x0+i,y0,color);}              //draw horizontal line   assume:x0<x1,y0<y1
		  for (uint8_t i=0; i<=y1-y0;i++)
		  {    setpixel(buff,x1,y0+i,color);}              //draw vertical line		           
}
...
void fillrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	...
	{
		drawline(buff,x+1,y+i,x+w-1,y+i,color);
	}
}
...
void drawrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
		drawline(buff,x,y, x+w,y+h, color);            //draw w
		drawline(buff,x,y+1,x,y+h,color);
		drawline(buff,x+1,y+h,x+w-1,y+h,color);
...
void clear_circle(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t r,uint8_t color) {
	for (uint8_t i=0;i<=128;i++)
	{
		for (uint8_t j=0;j<=64;j++)
		{
...
void clearstring(uint8_t *buff, uint8_t x, uint8_t line, uint8_t *c)  {
	for(uint8_t i=0;i<strlen(c);i++)
	{
		drawchar(buff,x+5*i,line, '\0');
	}
}
//clear string

void clearrec(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color){
	for (uint8_t i = 0; i < w+1 ; i++)
	{
		for (uint8_t j = 0; j < h+1 ; j++)
		{
			clearpixel(buff,x+i,y+j);
		}
